...
public class DataBase implements Constantes {
	...
	public DataBase () {
		try{
			...
			if (_DEBUG) System.out.println("Niveau max possible : " + _maxLvl);
			br.close();
	...
	public boolean loadLevel (int number) {
		...
		loadXml("levels/level" + ((number < 10) ? "0" + number : number) + ".xml");
		if (_DEBUG) System.out.println("Nombre d'item a charger : " + _item_in_file.getLength());
		...
		for (int i = 0; i < _item_in_file.getLength(); ++i) {
			...
			switch (Integer.parseInt(cur_item.item(1).getTextContent())) {
				...
				default :
					if (_DEBUG) System.out.println("Erreur de chargement de l'item " + i);
					result = false;
				break;
			}
		}
	...
	public boolean loadSave (int number) {
		...
		loadXml("saved_game/level" + ((number < 10) ? "0" + number : number) + ".xml");
		if (_DEBUG) System.out.println("Nombre d'item a charger : " + _item_in_file.getLength());
		...
		for (int i = 0; i < _item_in_file.getLength(); ++i) {
			...
			switch (Integer.parseInt(cur_item.item(1).getTextContent())) {
				case 1 :
					img = cur_item.item(11).getTextContent();
					width = Integer.parseInt(((cur_item.item(7).getTextContent().isEmpty()) ? "0" : cur_item.item(7).getTextContent()));
					height = Integer.parseInt(((cur_item.item(9).getTextContent().isEmpty()) ? "0" : cur_item.item(9).getTextContent()));
				...
				case 6 :
					...
					flight = Double.parseDouble(cur_item.item(9).getTextContent());
					stat = Double.parseDouble(cur_item.item(11).getTextContent());
					takeoff = Double.parseDouble(cur_item.item(13).getTextContent());
					// switched contient le fait que l'oiseau bouge ou non
					switched = Boolean.parseBoolean(cur_item.item(15).getTextContent());
					
					Fenetre._list_birds.add(new Colibri(posx, posy, width, flight, stat, takeoff, switched));
				break;
				...
				case 7 :
					...
					Fenetre._list_birds.add(new Pigeon(posx, posy, width, flight, stat, takeoff, switched));
				break;
				...
				case 8 :
					...
					Fenetre._list_birds.add(new Moineau(posx, posy, width, flight, stat, takeoff, switched));
				break;
				
				case 9 :
					Fenetre._list_egg.add(new Small_egg(posx, posy));
				break;
				
				case 10 :
					Fenetre._list_egg.add(new Normal_egg(posx, posy));
				break;
				
				case 11 :
					Fenetre._list_egg.add(new Big_egg(posx, posy));
				break;
	...
	public boolean savelevel () {
		String NomFichier = "saved_game/level" + ((_currentLvl < 10) ? "0" + _currentLvl : _currentLvl) + ".xml";
		...
		try {
			...
			for (int i = 0; i < Fenetre._list_static_items.size(); ++i) {
				...
				out.println("\t\t<width>" + Fenetre._list_static_items.get(i).getWidth() + "</width>");
			...
			for (int i = 0; i < Fenetre._list_egg.size(); ++i) {
				out.println("\t<ItemDisplay>");
				out.println("\t\t<type>" + Fenetre._list_egg.get(i).getType() + "</type>");
				out.println("\t\t<posx>" + Fenetre._list_egg.get(i).getPosX() + "</posx>");
				out.println("\t\t<posy>" + Fenetre._list_egg.get(i).getPosY() + "</posy>");
				out.println("\t</ItemDisplay>");
			}
			...
			if (_DEBUG) System.out.println("Sauvegarde le niveau a l'instant t");
	...
	public boolean saveExist(int lvl) {
		File fichier = new File("saved_game/level" + ((lvl < 10) ? "0" + lvl : lvl) + ".xml");
		return fichier.exists();
	}
	
	public boolean nextLvlExist() {
		File fichier = new File("levels/level" + ((_currentLvl + 1 < 10) ? "0" + (_currentLvl + 1) : (_currentLvl + 1)) + ".xml");
		return fichier.exists();
	}
	...
	*/
	...
	public void saveHighScore() {
		...
		if (Fenetre._score.getHigh() < Fenetre._score.getCurrent()) {
			...
			try {
				...
				if (!curLvlExist) {
					out.println("\t<Score>");
					out.println("\t\t<lvl>" + _currentLvl + "</lvl>");
					out.println("\t\t<highscore>" + Fenetre._score.getCurrent() + "</highscore>");
					out.println("\t</Score>");
				}
	...
	private void clearLvl() {
		...
		Fenetre._list_egg = new LinkedList<Eggs>();
	}
	
	private void updateMaxLvl() {
		if (_currentLvl == _maxLvl) {
			if (nextLvlExist()) ++_maxLvl;
	...
	public void deleteSave(int lvl) {
		File save = new File("saved_game/level" + lvl + ".xml");
		save.delete();
	}
}
